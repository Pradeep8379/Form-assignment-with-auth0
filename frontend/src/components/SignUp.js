...
export const SignUp = () => {
    ...
    const handleLogout = async () => {
        ...
        try {
            await axios.post(
                ...
                { email:user.email,start:user.updated_at },
    ...
    return (
        <>
            {isAuthenticated ? <div className="d-flex justify-content-center mt-5"><button className=' btn btn-primary' onClick={() => handleLogout()}>Log Out</button></div> :
